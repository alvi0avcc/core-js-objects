 ...
 */
function sellTickets(queue, m25 = 0, m50 = 0) {
  if (queue.length === 0) return true;
  const money = queue[0];
  if (money === 25) {
    return sellTickets(queue.slice(1), m25 + 1, m50);
  }
  if (money === 50) {
    if (m25 === 0) {
      return false;
    }
    return sellTickets(queue.slice(1), m25 - 1, m50 + 1);
  }
  if (money === 100) {
    if (m50 > 0 && m25 > 0) {
      return sellTickets(queue.slice(1), m25 - 1, m50 - 1);
    ...
    if (m25 >= 3) {
      return sellTickets(queue.slice(1), m25 - 3, m50);
    }
    return false;
  }
  return false;
...
class Selector {
  ...
  element(value) {
    return this.updateSelector(value, 1, 'element');
  }
  ...
  pseudoElement(value) {
    return this.updateSelector(`::${value}`, 6, 'pseudoEl');
  }

  combine(selector1, combinator, selector2) {
    const sel1 = selector1.stringify();
    const sel2 = selector2.stringify();
    this.css = `${sel1} ${combinator} ${sel2}`;
    return this;
  }
  ...
  updateSelector(value, order, key) {
    if (this.order > order)
      throw new Error(
